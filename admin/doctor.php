		   ...
		   <?php
                        if(isset($_POST['submit2']))
                        {
                          ...
                          $email=$_POST['email'];
						   ...
						   $result=mysqli_query($connect,"Select * from ip_doctor where uid='$vid'");
						    $rowcount=mysqli_num_rows($result);
                            if($rowcount<=0)
                            {
                               $query=mysqli_query($connect,"insert into ip_doctor(uid,fullname,username,pass,email,mobile,address,specialisation) values('$vid','$fullname','$username','$pass','$email','$mobile','$address','$specialisation')");
                                    if(mysqli_affected_rows($connect))
                                    {
                                        echo '<div class="alert alert-success">Doctor User is added Successfully.</div>';
                                    }
                                    else
                                    {
                                ...
                                {
                                    echo '<div class="alert alert-warning">Doctor User ID Should be Unique</div>';
                                }
                            }
                ...
                <div class="form">
                  <form class="form-validate form-horizontal " id="register_form" method="post" action="">
                    <div class="form-group ">
                      <label for="vid" class="control-label col-lg-2">Doctor User ID <span class="required">*</span></label>
                      <div class="col-lg-10">
					    <?php

$Reg_Result = mysqli_query($connect,"select * from ip_doctor ORDER BY id desc");
$Reg_Row = @mysqli_fetch_array($Reg_Result,MYSQLI_ASSOC);
?>
					  
                        <input class=" form-control" id="vid" value="<?php echo $Reg_Row['id']+1; ?>" name="vid" type="text" readonly />
                      </div>
                    </div>
